...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!currentPassword || !newPassword) {
      throw createApiError('Current password and new password are required', HttpStatus.BAD_REQUEST, ErrorTypes.VALIDATION_ERROR);
    }

    if (newPassword.length < 6) {
      throw createApiError('New password must be at least 6 characters long', HttpStatus.BAD_REQUEST, ErrorTypes.VALIDATION_ERROR);
    }
    ...
    if (!userDoc.hasPassword || !userDoc.password) {
      throw createApiError('No password set. Please set a password first.', HttpStatus.BAD_REQUEST, ErrorTypes.VALIDATION_ERROR);
    }
    ...
    logError(error, 'POST /api/auth/change-password');
